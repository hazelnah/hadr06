...
{ 
  ...
  fSizeWorld = new G4UIcmdWithADoubleAndUnit("/testhadr/det/setWorldSize",this);
  fSizeWorld->SetGuidance("Set size of the world volume");
  fSizeWorld->SetParameterName("WorldSize",false);
  fSizeWorld->SetRange("WorldSize>0.");
  fSizeWorld->SetUnitCategory("Length");
  fSizeWorld->AvailableForStates(G4State_PreInit,G4State_Idle);

  fSizeWall = new G4UIcmdWithADoubleAndUnit("/testhadr/det/setWallSize",this);
  fSizeWall->SetGuidance("Set size of the wall");
  fSizeWall->SetParameterName("WallSize",false);
  fSizeWall->SetRange("WallSize>0.");
  fSizeWall->SetUnitCategory("Length");
  fSizeWall->AvailableForStates(G4State_PreInit,G4State_Idle);
...
{ 
  ...
  if( command == fSizeWorld )
   { fDetector->SetWorldSize(fSizeWorld->GetNewDoubleValue(newValue)); }

  if( command == fSizeWall )
   { fDetector->SetWallSize(fSizeWall->GetNewDoubleValue(newValue)); }
