...
G4VPhysicalVolume* DetectorConstruction::ConstructVolumes()
{
  ...
  PrintParameters();
  // Sensitive detector
  CreateDetectorWall(lWorld);
  //always return the root volume
  //
  return fPWorld;
}

void DetectorConstruction::CreateDetectorWall(G4LogicalVolume* logicWorld)
{
  ...
  auto solidDetector = new G4Box("solidDetector", fDetectorSizeX/fDetectorRows, fDetectorSizeY/fDetectorCols, fDetectorSizeZ);
  ...
  {
    for(G4int j = 0; j < fDetectorCols; j++)
    {
...
void DetectorConstruction::SetWallSize(G4double value)
{
  fDetectorSizeX = value;
  fDetectorSizeY = value;
  G4RunManager::GetRunManager()->ReinitializeGeometry();
